...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Check Splitwise auth status
      try {
        const authRes = await fetch("/auth/splitwise/status");
        const authData = await authRes.json();
        setIsSplitwiseAuth(authData.is_authenticated);
      } catch {
        // Fallback or ignore
      }

      // Fetch Kotak balance (best-effort, don't fail if not available)
      try {
        const balRes = await fetch("/api/accounts/kotak/balance");
        const balData = await balRes.json();
        setKotakBalance(balData.balance || null);
      } catch {
        // ignore
      }
          ...
          {kotakBalance && (
            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-3 py-1.5">
              ...
              <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">
                {formatCurrency(
                  kotakBalance.amount_minor_units,
                  kotakBalance.currency_code,
                )}
              </span>
              <span className="text-[10px] text-muted-foreground">
                Kotak · as of {kotakBalance.balance_date}
              </span>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-md font-bold text-muted-foreground uppercase tracking-tight">
                        {txn.institution}
                      </span>
                    </div>
                    {isSelectable && (
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 max-w-[200px]">
                    Authenticate with Splitwise to see your expenses and start
                    reconciliation.
                  </p>
                </div>
                <button
                  onClick={() =>
                    (window.location.href = "/auth/splitwise/login")
                  }
                  className="w-full py-2.5 bg-primary text-primary-foreground font-bold rounded-xl hover:bg-primary/90 transition shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
                >
